...
class GPT2Model(nn.Module):
    ...
    def from_pretrained(cls, model_type : MODEL_TYPES):
        ...
        config = {
            ...
            # 350M Params
            'gpt2-medium' : GPT2Model.Config(
                n_layer=24,
                n_head=16,
                n_embd=1024
            ),
            # 774M Params
            'gpt2-large' :    GPT2Model.Config(
                n_layer=36,
                n_head=20,
                n_embd=1280
            ),
            # 1558M Params
            'gpt2-xl' :    GPT2Model.Config(
                n_layer=48,
                n_head=25,
                n_embd=1600
            ),
    ...
    def forward(self, idx : Tensor):
        ...
        assert seq_len <= self.config.block_size , f'Cannot forward sequecne longer than block size {self.config.block_size}'
        
        # forward the token and positional embeddings
        pos_input = torch.arange(0, seq_len, dtype=torch.long, device=idx.device) # (seq_len, n_embd)
...
class CasualSelfAttention(nn.Module):
    ...
    def __init__(self, config : GPT2Model.Config) -> None:
        ...
        self.register_buffer('bias', 
                             tril(ones(config.block_size, config.block_size)
    ...
    def forward(self, x : Tensor):
        ...
        # recombine the heads
        embeddings = embeddings.transpose(1, 2).contiguous().view(batch_size, seq_len, embed_dim)
